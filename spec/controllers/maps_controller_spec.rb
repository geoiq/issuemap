...
describe MapsController do
  it { should route(:get, '/maps').to(:action => :index) }
  it { should route(:get, '/maps/new').to(:action => :new) }
  it { should route(:post, '/maps').to(:action => :create) }
  it { should route(:get, '/maps/1/edit').to(:action => :edit, :id => 1) }
  it { should route(:put, '/maps/1').to(:action => :update, :id => 1) }
  it { should route(:get, '/maps/verify').to(:action => :verify) }
  ...
  context "POST to :create" do
    before(:each) do
      request.env["HTTP_REFERER"] = verify_maps_path
